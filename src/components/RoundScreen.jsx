...
export default function RoundScreen({ round, onTimeUp }) {
  ...
  useEffect(() => {
    ...
    // Emit that we're ready for round (triggers server to send round_started)
    if (code) {
      socket.emit('start_round', { code })
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <div className="mono" style={{ color: '#4ade80', fontSize: '0.7rem', letterSpacing: '0.4em', marginBottom: '0.4rem' }}>
            [ DISCUSSION PHASE ]
          </div>
        ...
        <div style={{ position: 'relative', width: '280px', height: '280px' }}>
          <svg width="280" height="280" style={{ transform: 'rotate(-90deg)', position: 'absolute' }}>
            <circle cx="140" cy="140" r="120" fill="none" stroke="#0a1a10" strokeWidth="8" />
          ...
          <div style={{
            ...
            <span className="mono" style={{
              ...
              textShadow: isLow ? '0 0 30px rgba(239,68,68,0.9)' : '0 0 30px rgba(74,222,128,0.8)',
              transition: 'color 0.5s, text-shadow 0.5s'
            }}>
              {String(minutes).padStart(2, '0')}:{String(seconds).padStart(2, '0')}
            </span>
            <span className="mono" style={{ fontSize: '0.65rem', letterSpacing: '0.3em', color: isLow ? '#dc2626' : '#15803d' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {alivePlayers.map((p, i) => (
              <div key={i} style={{
                ...
                <span className="mono" style={{ color: '#86efac', fontSize: '0.8rem' }}>{p.name}</span>
              </div>
            ))}
          </div>
        </div>
